...
```python
from cdc_1c import HandlerRunner
...
ochered = HandlerRunner(engine=engine, schema="cdc_1c", handler=OtpravkaVOchered(queue="cdc"))

replicator = Replicator1C(..., engine=engine, handler_runners=[zakazy, ochered])
```

`HandlerRunner` — исполнитель **одного** обработчика: его состояние, его очередь и его поток.
По раннеру на обработчика, чтобы тяжёлая витрина не задерживала остальные.
...
потоков полной выгрузки и по потоку на каждого обработчика. Занять чужие потоки они не могут, но
`engine` у них общий, и одновременно держать соединение могут все сразу:

```
pool_size >= full_load_workers + число обработчиков + 1
```

Дефолтный для SQLAlchemy `pool_size=5` покрывает это при двух воркерах и двух обработчиках. Если
добавляете обработчиков или воркеров, поднимайте и пул: не хватает соединений — кто-то встаёт в
ожидание, и затормозить может как обработчик, так и сама выгрузка.

### Несколько планов обмена

Планов обмена может быть несколько — тогда на каждый заводится свой `Replicator1C`. Если обработчик
подписан на таблицы, которые наполняют **разные** репликаторы (витрина соединяет регистр из одного
плана со справочником из другого), им нужен **общий `HandlerRunner`**:
...
# ТЕ ЖЕ объекты раннеров уходят обоим репликаторам — этого достаточно.
rep1 = Replicator1C(..., exchange_name="План1", queue_guid=GUID1, engine=engine,
                    handler_runners=[zakazy])
rep2 = Replicator1C(..., exchange_name="План2", queue_guid=GUID2, engine=engine,
                    handler_runners=[zakazy])

with ThreadPoolExecutor(max_workers=2) as pool:
    for rep in (rep1, rep2):
        pool.submit(rep.run_forever, interval=60)
```

Реестр незавершённых merge заводить самому не нужно: его раздаёт первый репликатор, а второй,
получив те же раннеры, подхватывает уже присоединённый вместо своего. Свести вместе раннеры,
присоединённые к разным реестрам, нельзя — это ошибка на старте, а не молчаливая потеря строк.

Репликаторы равнозначны: каждый на входе в `run_forever` заявляет себя пользователем раннеров, на
выходе снимает заявку, и поток обработчика гаснет, когда выйдет последний. `SIGTERM`/`SIGINT`
останавливают **все** циклы сразу, в каком бы потоке они ни крутились; отдельный цикл можно
остановить программно через `rep.request_stop()`.

**Почему один процесс, а не по процессу на план.** И грязные отметки, и реестр незавершённых merge
живут в памяти. Обработчик в другом процессе не только не узнает о чужих изменениях — он не увидит
и незакоммиченные merge соседа, а их строки уже имеют `merged_on` в прошлом. Граница окна не будет
к ним прижата, отметка их перешагнёт, и строки **потеряются молча**. Поэтому репликаторы, делящие
обработчик, обязаны жить в одном процессе, на одном `engine` и в одной схеме — несовпадение
проверяется на старте и роняет сборку.

Если же планы **не пересекаются** и каждый обработчик читает таблицы только своего плана, разносить
по процессам можно свободно: делить нечего.

Пул считается по сумме: `sum(full_load_workers) + число репликаторов + число обработчиков`.
