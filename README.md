...
# почему столько, см. README_HANDLERS.md, «Сколько нужно соединений к БД»
...
дополнительных репликаторов (Replicator) для других планов обемна можно посмотреть в этом файле: [runner.py](config/runner.py)
...
`-v "$PWD/config:/config:ro"` — монтирует папку config в текщем каталоге. в этой папке будет содержаться файл [runner.py](config/runner.py), а также подпапка с примерами обработчиков (config/handlers).
...
- Полная выгрузка спроектирована так, чтобы работать параллельно с получением изменений объекта и не презатирать свежие изменения объекта. Также она автоматически разбивает объект на отрезки по времени и выгружает его порциями с автоматическим подбором размера (batch_size).
...
`Replicator1C` читает изменения и пишет их в БД, `Handler1C` + `HandlerLoop` дают запуск своего кода
...
справочником по guid. Для DWH обычно удобнее менее строгая нормализация — витрины, собранные
заранее.

Считать их вхолостую по расписанию не нужно: репликатор знает, когда данные приехали, и сам зовёт
ваш код. Такой код называется **обработчиком** — это класс с двумя строчками объявления:
...
Как это устроено и что нужно знать, чтобы витрина считалась правильно, — в
[README_HANDLERS.md](README_HANDLERS.md). Готовые разобранные примеры — в
