...
Остальные — необязательные (пользователь OData, схемы, число потоков выгрузки, период опроса,
режим, уровень лога, `TZ`). **Полный справочник со значениями по умолчанию, поведением при
ошибках конфигурации и примерами — [README_ENV.md](README_ENV.md).**
...
**Без своего кода.** Контейнер запускает тот же entrypoint `cdc-1c` и читает те же переменные
(справочник — [README_ENV.md](README_ENV.md)). Первый запуск удобно сделать без `-d`, чтобы лог
шёл прямо в терминал:
...
docker run --rm --network host \
  ...
  -e CDC1C_DB_URL="postgresql+psycopg2://user:pass@localhost:5432/dwh" \
  -e CDC1C_DB_SCHEMA=cdc_1c -e CDC1C_DB_TEMP_SCHEMA=cdc_1c_tmp \
  -v "$PWD/config:/config:ro" \
  sobolevp/cdc-1c:latest
```

`-v "$PWD/config:/config:ro"` — проброс своего кода: лежит в каталоге `runner.py` — запускается он,
нет каталога — контейнер работает по переменным выше. Ключ стоит в примере сразу, чтобы при
появлении обработчиков команду не пришлось переписывать.

`server/base`, `user:pass` и прочее здесь — заглушки, подставьте свои. Адрес БД оставлен
`localhost` намеренно: он парный к `--network host` ниже.
...
`handlers/` кладутся в тот самый каталог `./config`. Стартовый шаблон лежит в самом образе —
репозиторий клонировать не нужно:

```bash
docker run --rm sobolevp/cdc-1c:latest tar c -C /opt/cdc-1c config | tar x
# правим config/runner.py и config/handlers/ — команда запуска остаётся прежней
```

Как есть шаблон поднимает только репликатор: обработчики и расписания в нём закомментированы. Это
обычный python-скрипт, поэтому любое значение в нём можно задать литералом вместо переменной
окружения. Файл названием отличается от `runner.py` — укажите путь в `CDC1C_RUNNER`.
...
табличные части документа, весь набор движений регистратора).

Глубокий объект библиотека дополнительно режет на месяцы: `$skip` в 1С дорог (на каждый запрос
выборка строится заново и сортируется целиком), а фильтр по дате переводит запрос на индекс.
Включается это по факту — если объект не дочитался за десяток страниц; мелкие объекты читаются
как раньше, без единого лишнего запроса.

Как всё это устроено внутри — подбор размера страницы, нарезка на периоды, почему keyset-курсор
в 1С неприменим к ссылочным ключам и как снимок разъезжается с изменениями — в
[DESIGN.md](DESIGN.md): документ для тех, кто правит код или разбирается в поведении на своей базе.
...
Метка, а не булев флаг, — из-за сигнала, пришедшего ПОСЕРЕДИНЕ прогона: поднять `true` над `true`
нельзя, следа не остаётся, и прогон снимал флаг вместе с непрочитанным сигналом — витрина отставала
до следующего изменения, молча. Метку прогон снимает, только если она не правее границы его окна
(`update_requested_at <= boundary`); всё, что правее, он не читал, и оно заводит его ещё раз.
...
| `update_requested_at` | момент сигнала: репликатор увидел изменение подписанной таблицы; `NULL` — незакрытых сигналов нет |
