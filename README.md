...
Параметры проверяются в конструкторе: адрес без схемы, строка подключения вместо `engine`,
`("user",)` вместо пары `(user, password)`, код узла вместо `Ref_Key` — всё это падает сразу и с
текстом, что передать, а не ошибкой 1С посреди первого цикла.
