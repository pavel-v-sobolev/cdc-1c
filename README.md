...
Обработчик получит окно с начала времён (`context.last_run_at` = `None`) и `context.full_rebuild` =
`True`. Первый в жизни прогон — тоже пересборка, флаг для этого ставить не нужно. Обработчику,
который считает только по окну, `full_rebuild` не нужен вовсе; он пригодится, если пересборка
устроена иначе, чем обычный прогон — скажем, идёт по частям, чтобы не делать один гигантский merge.
