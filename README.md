...
### `merges_in_process_1c` — незавершённые merge

Строка живёт ровно столько, сколько идёт один merge: появляется перед ним и исчезает после коммита.
...
| `replicator` | чей это merge — имя плана обмена (у обработчика `handler:<имя>`) |
| `object_name` | имя таблицы в БД, которую пишет merge |
...
незавершённые merge в общую `merges_in_process_1c`.
...
Верхняя граница окна при этом тоже берётся из базы — из таблицы `merges_in_process_1c`, куда
