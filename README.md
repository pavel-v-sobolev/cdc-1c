...
Параметры присваиваются явно, по одному: и в python-приложении, и в контейнере сборка выглядит
одинаково — меняется только источник значений (литералы или `os.environ`). Готовый пример точки
входа для контейнера — [example_config/runner.py](example_config/runner.py); если свой код не нужен
вовсе, есть entrypoint `python -m cdc_1c`, читающий те же параметры из переменных `CDC1C_*`.
...
Обработчики — обычные импортируемые модули, поэтому лежат где угодно, а общий код между ними
подключается обычным `import`. Обновление библиотеки этот код не трогает.
...
граница окна, поэтому перечислять надо **все** таблицы, из которых обработчик выбирает данные.
...
- [zakazy_klientov_grouped.py](example_config/handlers/zakazy_klientov_grouped.py) — ключ меняется
  (`GROUP BY` по номеру документа и артикулу), но пересчёт остаётся инкрементальным.
...
хотя бы по одному источнику (`self.changed_since(ctx, ...)`).
...
На больших объёмах у такого `OR` есть известная беда, и она не про отсутствие индексов: ветки живут
на **разных** таблицах соединения, поэтому ни по одной нельзя отфильтровать до `JOIN` — условие
проверяется на готовой паре и вырождается в `Join Filter` поверх полного соединения. `BitmapOr` из
индексных сканов Postgres строит только когда все ветки `OR` на одной таблице, а переписать `OR` в
`UNION` через границу джойна он не умеет. Делается это руками: запрос «какие группы изменились»
разворачивается в `UNION` по одному подзапросу на источник — каждый ведётся своей отфильтрованной
таблицей и заходит через её индекс `merged_on` (эти индексы репликатор создаёт сам). Стоимость тогда
зависит от размера окна, а не от размера таблиц.
