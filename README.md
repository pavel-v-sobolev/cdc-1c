...
запускается в своём (пример — [example_config/runner_multi.py](example_config/runner_multi.py)).
`SIGTERM`/`SIGINT` останавливают **все** циклы сразу, в каком бы потоке они ни крутились; отдельный
цикл можно остановить программно через `rep.request_stop()`.
...
поднять отдельным процессом или вообще в другом контейнере — связывает всех одна общая БД. Готовый
пример такого процесса — [example_config/runner_handlers.py](example_config/runner_handlers.py).
...
Единица блока — та же, по которой вы удаляете. Если витрина удаляется группами, блок обязан брать
группы целиком: отрезать от группы «строки месяца» нельзя, иначе `delete_condition` снесёт её
остаток. Оба примера нарезаны по месяцам, и из-за этого правила выглядят по-разному:

- [zakazy_klientov.py](example_config/handlers/zakazy_klientov.py) — ключ витрины совпадает с ключом
  источника, поэтому блок берёт из источника весь месяц и весь месяц в витрине и заменяет. Заодно
  это чинит переезд строки между месяцами: из старого месяца её удалит его блок, в новом создаст
  его собственный.
- [zakazy_klientov_grouped.py](example_config/handlers/zakazy_klientov_grouped.py) — ключ витрины
  вычисляется из данных, и месяц режет группу пополам. Поэтому блок отбирает **группы** — тем же
  запросом PREVIOUS/CURRENT, что и инкремент, только с отбором «что лежит в этом месяце» вместо
  «что изменилось за окно». Без половины PREVIOUS пересборка была бы неполной: группу, которую
  покинули ещё до её начала, не нашёл бы ни один блок.
