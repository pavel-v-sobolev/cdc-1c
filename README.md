...
`runner.py` (и `runner_cron.py` — то же самое плюс полные выгрузки по расписанию) и пакетом
`handlers/` рядом. Запускается как обычный скрипт:
...
Готовая точка входа с расписаниями — [example_config/runner_cron.py](example_config/runner_cron.py).
