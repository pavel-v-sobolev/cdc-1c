...
Что именно cdc-1c создаёт в вашей базе: таблицы данных, служебные поля строк, служебные таблицы —
...
- У регистра бухгалтерии есть ещё две колонки, `ExtDimensionsDr` и `ExtDimensionsCr` (`jsonb`) —
  субконто проводки. Ключ — **вид субконто**, значение — `{"value": …, "type": …}`:

  ```sql
  -- проводки с субконто «Статьи затрат» = «Оплата труда» по дебету
  WHERE "ExtDimensionsDr" @> '{"6a6ada17-…": {"value": "e5b0e2f0-…"}}'
  ```
...
```
pool_size >= full_load_workers + 2 + число обработчиков + число расписаний
```
